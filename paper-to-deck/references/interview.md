...
## Category H · Medical-teaching visual options (ask AFTER A–G, BEFORE Typography)

These five questions shape the visual vocabulary of the deck. They map onto the archetypes in `references/slide-patterns.md` under "Medical-teaching visual archetypes". Ask all five together as one block; users who want defaults can answer `f` on each.

### H1. Visual scheme?
- a) **Crimson-blue** — cover title in deep blue (`#1D4E89`), red accent for eyebrows and emphasis. Matches deck-1 reference flavour.
- b) **Teal** — cover block and branded footer in teal (`#1E4A5F`), red accent retained. Matches deck-2 reference flavour.
- c) **Minimal** — pure black/white body, `text.muted` for citations, no pastel fills, no red accent. Use for formal conference talks.
- d) Decide for me — I'll use crimson-blue (default).

### H2. Cover template?
- a) **V1a mascot** — eyebrow + big title + lower-left mascot + lower-right author block. Good for lab meeting / journal club.
- b) **V1b teal-block** — top third teal block with title, bottom white with institution + author. Good for formal department seminars.
- c) **V1c minimal** — title + author only. Use for serious primary trials (mortality RCT, safety study) where mascot levity is inappropriate.
- d) Decide for me — I'll use V1a mascot.

### H3. Branded footer strip?
> Text to display at the bottom of every body slide (e.g. "College of Medicine, National Cheng Kung University", "Division of Infectious Diseases", "Lab of XYZ"). Blank = no footer.
- a) Use my institution — `<fill in>`
- b) Use my division — `<fill in>`
- c) Skip — no footer
- d) Decide for me — I'll skip.

### H4. Body-text language?
> Slide titles default to English per Category E. This question controls **body** language on content slides.
- a) **English** — all body bullets and captions in English.
- b) **Chinese** — body bullets and captions in Traditional Chinese (keeps titles English unless E1 was also Chinese).
- c) **Mixed** — English for key terms (highlighted in `accent.red`), Chinese for teaching narrative. Matches deck-2 reference flavour.
- d) Decide for me — I'll match Category E (same as slide language).

### H5. Contextual public-domain imagery?

> I've scanned the paper and identified `<N>` places where a public-domain image (historical photo, guideline screenshot, anatomical illustration, epidemiology map) would strengthen the slide. Allowed sources: Wikimedia Commons, NIH Open-i, CDC Public Health Image Library, WHO open imagery. All fetched images are checked for CC licence before use.

> **Candidates I've found:**
> `<auto-generated list from scripts/extract_paper.py imagery_candidates.json, each with {slide, rationale, suggested query}>`

- a) **Fetch all** — strict licence filter (CC0 / CC BY / CC BY-SA / US-gov PD).
- b) **Fetch all** — loose licence filter (also allow CC BY-NC; avoid if deck will be recorded commercially).
- c) **Let me pick** — I'll tick which candidates to pursue.
- d) Skip — no external imagery.
- e) Decide for me — I'll skip (safe default).

> Note: pop-culture memes (Shrek, movie stills, etc.) are copyrighted and the skill will **not** fetch them automatically. If a candidate is flagged `manual-only`, the skill leaves a placeholder and you paste the meme yourself before final export.

---

## Category F · Typography (mandatory, ask AFTER all category A–H questions)
...
**Medical-teaching visual options** (Category H)
Q13. ... (visual scheme)
Q14. ... (cover template)
Q15. ... (branded footer)
Q16. ... (body language)
Q17. ... (public-domain imagery batch)
